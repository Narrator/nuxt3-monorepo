...
export const getImage = (src, { modifiers, baseURL = '/_nuxt/image' } = {}, ctx) => {
  ...
  if (!width) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.warn(`A defined width should be provided to use the \`aws_amplify\` provider. Defaulting to \`${largestWidth}\`. Warning originated from \`${src}\`.`)
    }
